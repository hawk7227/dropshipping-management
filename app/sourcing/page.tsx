...
// ── Product Grid Components (from Products page) ────────────────────
...
// ── Dark theme (same as products page) ──────────────────────────────
...
// ============================================================
// API RESPONSE MAPPER (for Product Card Grid)
// ============================================================
...
export default function SourcingEngine() {
  ...
  // ── Fetch products for the card grid ────────────────────────────────
  const fetchGridProducts = useCallback(async () => {
    setGridLoading(true);
    try {
      ...
      if (result.success) {
        ...
        const mapped = (Array.isArray(productsArray) ? productsArray : [])
          .map((item: Record<string, unknown>) => mapApiProduct(item));
        setGridProducts(mapped);
      }
    } catch (err) {
      console.error('[SourcingPage] Failed to fetch grid products:', err);
    } finally {
  ...
  const handleGridSelectToggle = useCallback((id: string) => {
  ...
  const handleGridSelectAll = useCallback(() => {
    setGridSelectedIds(new Set(gridProducts.map(p => p.id)));
  }, [gridProducts]);

  const handleGridDeselectAll = useCallback(() => {
    setGridSelectedIds(new Set());
  }, []);

  // ── Grid Action Handlers ────────────────────────────────────────────
  const handleGridViewDetails = useCallback((product: Product) => {
    console.debug('[SourcingPage] View details:', product.id, product.title);
  }, []);

  const handleGridRefresh = useCallback(async (id: string) => {
    try {
      const response = await fetch(`/api/products?action=get&id=${id}`);
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const result = await response.json();
      if (result.success && result.data) {
        await fetchGridProducts();
      }
    } catch (err) {
      console.error('[SourcingPage] Refresh failed:', err);
    }
  }, [fetchGridProducts]);

  const handleGridPause = useCallback(async (id: string) => {
    ...
    try {
      ...
      console.error('[SourcingPage] Pause failed:', err);
    }
  }, [gridProducts, fetchGridProducts]);

  const handleGridRemove = useCallback(async (id: string) => {
    try {
      const response = await fetch('/api/products?action=bulk-status-update', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      console.error('[SourcingPage] Remove failed:', err);
    }
  }, [fetchGridProducts]);

  const handleGridSyncShopify = useCallback(async (id: string) => {
    try {
      ...
      console.error('[SourcingPage] Shopify sync failed:', err);
    }
  }, [fetchGridProducts]);

  const handleGridBulkSync = useCallback(async (ids: string[]) => {
    try {
      const response = await fetch('/api/products?action=sync-shopify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      console.error('[SourcingPage] Bulk sync failed:', err);
    }
  }, [fetchGridProducts]);

  const handleGridBulkActivate = useCallback(async (ids: string[]) => {
    try {
      await fetch('/api/products?action=bulk-status-update', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      console.error('[SourcingPage] Bulk activate failed:', err);
    }
  }, [fetchGridProducts]);

  const handleGridBulkPause = useCallback(async (ids: string[]) => {
    try {
      await fetch('/api/products?action=bulk-status-update', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ productIds: ids, status: 'paused' }),
      });
      await fetchGridProducts();
      setGridSelectedIds(new Set());
    } catch (err) {
      console.error('[SourcingPage] Bulk pause failed:', err);
    }
  }, [fetchGridProducts]);

  const handleGridBulkExport = useCallback((ids: string[]) => {
    const exportProducts = gridProducts.filter(p => ids.includes(p.id));
    const csv = [
      'id,title,asin,status,cost_price,retail_price,profit_percent,category',
      ...exportProducts.map(p =>
        `"${p.id}","${(p.title || '').replace(/"/g, '""')}","${p.asin || ''}","${p.status}",${p.cost_price || ''},${p.retail_price || ''},${p.profit_percent?.toFixed(1) || ''},"${p.category || ''}"`
      ),
    ].join('\n');
  ...
  const handleGridBulkArchive = useCallback(async (ids: string[]) => {
    try {
      await fetch('/api/products?action=bulk-status-update', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ productIds: ids, status: 'archived' }),
      });
      await fetchGridProducts();
      setGridSelectedIds(new Set());
    } catch (err) {
      console.error('[SourcingPage] Bulk archive failed:', err);
    }
  }, [fetchGridProducts]);
  ...
  // Badges
  const ab = (a: SourcingProduct['avail']) => { const m: Record<string,[string,string]> = { in_stock:['bg-emerald-500/15 text-emerald-400','In Stock'], low_stock:['bg-amber-500/15 text-amber-400','Low'], oos:['bg-red-500/15 text-red-400','OOS'], unavail:['bg-red-500/15 text-red-400','Unavail'] }; const [c,l]=m[a]||m.unavail; return <span className={`text-[8px] px-1.5 py-0.5 rounded-full font-medium ${c}`}>{l}</span>; };
  const sb = (s: SourcingProduct['shopStatus']) => { const m: Record<string,[string,string]> = { none:['bg-zinc-500/10 text-zinc-500','None'], active:['bg-emerald-500/15 text-emerald-400','Active'], draft:['bg-amber-500/15 text-amber-400','Draft'], pushing:['bg-cyan-500/15 text-cyan-400','Pushing...'], archived:['bg-zinc-500/15 text-zinc-400','Archived'] }; const [c,l]=m[s]||m.none; return <span className={`text-[8px] px-1.5 py-0.5 rounded-full font-medium ${c}`}>{l}</span>; };
  ...
  return (
    <div className="products-dark min-h-screen p-4 lg:p-6" style={{ background:'#080810', color:'#e4e4e7', fontFamily:"'DM Sans',-apple-system,BlinkMacSystemFont,sans-serif" }}>
      ...
      <div className="max-w-[1800px] mx-auto">
        ...
        {tab === 'manual' && (<div className="space-y-4">
          ...
          {/* ══════════════════════════════════════════════════════════════ */}
          {/* PRODUCT CARD GRID — Below Shopify Sync Bar                   */}
          {/* ══════════════════════════════════════════════════════════════ */}
          <div className="mt-2">
            {/* View Toggle header (density switcher for 2/3/4/5 columns) */}
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-white">
                📦 Products ({gridProducts.length})
              </h3>
              <ViewToggle
                viewMode={viewMode}
            ...
            {/* Product Card Grid (identical to Products page) */}
            <ProductCardGrid
              products={gridProducts}
              density={gridDensity}
              isLoading={gridLoading}
              selectedIds={gridSelectedIds}
              onSelectToggle={handleGridSelectToggle}
              onSelectAll={handleGridSelectAll}
              onDeselectAll={handleGridDeselectAll}
              onViewDetails={handleGridViewDetails}
              onRefresh={handleGridRefresh}
              onPause={handleGridPause}
              onRemove={handleGridRemove}
              onSyncShopify={handleGridSyncShopify}
              onBulkSync={handleGridBulkSync}
              onBulkActivate={handleGridBulkActivate}
              onBulkPause={handleGridBulkPause}
              onBulkExport={handleGridBulkExport}
              onBulkArchive={handleGridBulkArchive}
            />
          </div>
        </div>)}
