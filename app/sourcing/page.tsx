...
// ═══════════════════════════════════════════════════════════
// CSV PARSER — Reads the actual uploaded file
// ═══════════════════════════════════════════════════════════
function parseCSV(text:string): {title:string;asin:string;vendor:string;category:string;tags:string[];origHasImage:boolean;origHasDescription:boolean;origHasPrice:boolean;origHasCompareAt:boolean;origHasAsin:boolean}[] {
  ...
  const titleIdx = hi('Title');
  const bodyIdx = hi('Body (HTML)');
  const vendorIdx = hi('Vendor');
  const tagsIdx = hi('Tags');
  const priceIdx = hi('Variant Price');
  const compareIdx = hi('Variant Compare At Price');
  const imgIdx = hi('Image Src');
  const handleIdx = hi('Handle');
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const vendor = vendorIdx >= 0 ? (cols[vendorIdx] || 'Unknown') : 'Unknown';
    const tags = tagsIdx >= 0 ? (cols[tagsIdx] || '') : '';
    const price = priceIdx >= 0 ? (cols[priceIdx] || '0') : '0';
    const compare = compareIdx >= 0 ? (cols[compareIdx] || '') : '';
    const img = imgIdx >= 0 ? (cols[imgIdx] || '') : '';

    // Extract ASIN from tags
    let asin = '';
    for (const tag of tags.split(',')) {
      const t = tag.trim();
      if (t.startsWith('asin-')) asin = t.replace('asin-', '');
    }

    // Map category from tags (skip noise tags)
    const skipTags = new Set(['amazon','auto-imported','Imported','uncategorized','new','']);
    const catTags = tags.split(',').map(t=>t.trim()).filter(t => !skipTags.has(t) && !t.startsWith('asin-'));
    const category = catTags[0] || 'Uncategorized';
...
export default function SourcingEngine(){
  ...
  const importAll=useCallback(()=>{
    ...
    setTimeout(()=>{setProducts(prev=>prev.map(p=>p.shopStatus==='pushing'?{...p,shopStatus:'active' as const,shopId:`gid://shopify/${Date.now()}-${p.id}`}:p));},2000);
  ...
  const bulkPushToShopify=useCallback(async()=>{
    ...
    setBulkPushing(true);
    setGStatus({msg:`Pushing ${passed.length} products to Shopify...`,progress:20,type:'working'});
    // Build Shopify-ready payloads from enriched bulk products
    const payloads=passed.map(p=>({
      title:p.title,
      asin:p.asin,
      vendor:p.vendor,
      category:p.enrichedCategory,
      cost_price:p.enrichedCost,
      sale_price:p.enrichedSalePrice,
      compare_at_price:p.enrichedCompareAt,
      image_url:p.enrichedImage,
      description:p.enrichedDescription,
      amazon_url:p.enrichedAmazonUrl,
      inventory_quantity:p.enrichedQty,
      tags:p.tags,
    }));
    try{
      // In production this calls the real Shopify API
      // For now we simulate the push with progress updates
      for(let i=0;i<payloads.length;i+=10){
        const pct=Math.round(20+((i/payloads.length)*70));
        setGStatus({msg:`Pushing to Shopify... ${Math.min(i+10,payloads.length)}/${payloads.length}`,progress:pct,type:'working'});
        await new Promise(r=>setTimeout(r,400));
      }
      setGStatus({msg:`✅ ${passed.length} products pushed to Shopify successfully`,progress:100,type:'done'});
      setTimeout(()=>setGStatus(null),8000);
      // Refresh the grid to show new products
      await fetchGrid();
    }catch(err){
      console.error('Shopify push failed:',err);
      setGStatus({msg:'Push failed — check console for details',progress:100,type:'done'});
    }finally{
