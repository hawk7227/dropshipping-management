...
// ═══════════════════════════════════════════════════════════
// AMAZON IMAGE FETCHER (client-side, no extra API route)
// Fetches real product images via a CORS proxy or direct patterns
// ═══════════════════════════════════════════════════════════
async function fetchAmazonImages(asin:string):Promise<string[]>{
  if(!asin)return[];
  const images:string[]=[];

  // Strategy 1: Use our own API route if it exists
  try{
    const res=await fetch(`/api/amazon-image?asin=${asin}`,{signal:AbortSignal.timeout(8000)});
    if(res.ok){
  ...
  // Strategy 2: Try to fetch Amazon page via our proxy API
  try{
    const res=await fetch(`/api/proxy?url=${encodeURIComponent(`https://www.amazon.com/dp/${asin}`)}`,{signal:AbortSignal.timeout(8000)});
    if(res.ok){
      ...
      // Extract hiRes from colorImages JSON
      const cm=html.match(/"hiRes"\s*:\s*"(https:\/\/[^"]+)"/g);
      if(cm) cm.forEach(m=>{const u=m.match(/"(https:\/\/[^"]+)"/);if(u?.[1]&&!images.includes(u[1]))images.push(u[1]);});
      // Extract data-old-hires
      const dh=html.matchAll(/data-old-hires="([^"]+)"/g);
      for(const m of dh){if(m[1]&&m[1].startsWith('http')&&!images.includes(m[1]))images.push(m[1]);}
      // CDN pattern matches
      const cdn=html.matchAll(/https:\/\/m\.media-amazon\.com\/images\/I\/[A-Za-z0-9+\-._]+\.(?:jpg|png)/g);
      for(const m of cdn){const u=m[0].replace(/\._[A-Z]{2}_[A-Z0-9_]+_\./,'._AC_SL1500_.');if(!images.includes(u))images.push(u);}
      if(images.length>0) return [...new Set(images)].slice(0,5);
    }
  }catch{}

  // Strategy 3: Construct known Amazon CDN URL patterns (works without scraping)
  // These are the most reliable direct URLs that don't require CORS
  const directUrls=[
  ...
  // Quick HEAD check to see if they return a real image
  for(const url of directUrls){
    try{
      const r=await fetch(url,{method:'HEAD',signal:AbortSignal.timeout(3000)});
      const ct=r.headers.get('content-type')||'';
      const cl=parseInt(r.headers.get('content-length')||'0',10);
      if(r.ok&&ct.startsWith('image/')&&cl>1000){images.push(url);break;}
    }catch{}
  }
...
async function enrichProduct(raw:ReturnType<typeof parseCSV>[0], idx:number, onStatus?:(msg:string)=>void): Promise<BulkProduct> {
  ...
  if (raw.origHasImage && raw.origImage) {
    ...
    if (imgs.length > 0) {
      ...
      flagReasons.push('Could not fetch image from Amazon');
    }
  } else {
...
export default function SourcingEngine(){
  ...
  const pushSel=useCallback(async()=>{
    ...
    setGStatus({msg:fail===0?`✅ ${ok} products pushed to Shopify!`:`⚠️ ${ok} pushed, ${fail} failed`,progress:100,type:'done'});
    setTimeout(()=>setGStatus(null),8000);try{await fetchGrid();}catch{}
  },[products,fetchGrid]);

  // ══════════════════════════════════════════════════════════
  // BULK IMPORT — Reads file, finds ASINs, fetches Amazon images
  ...
  const handleBulk=useCallback((e:React.ChangeEvent<HTMLInputElement>)=>{
    ...
    reader.onload=async(evt)=>{
      ...
      // Stage 2: Enrich each product (fetches real Amazon images)
      const enriched:BulkProduct[]=[];
      for(let i=0;i<parsed.length;i++){
        const pct=Math.round(15+((i/parsed.length)*65));
        const statusMsg=`[${i+1}/${parsed.length}] Fetching data for "${parsed[i].title.slice(0,45)}"${parsed[i].asin?` (${parsed[i].asin})`:''}...`;
  ...
  const bulkPushToShopify=useCallback(async()=>{
    ...
    setBulkPushing(true);
    setGStatus({msg:`Pushing ${passed.length} products to Shopify...`,progress:5,type:'working'});
    let ok=0,fail=0;const errs:string[]=[];

    for(let i=0;i<passed.length;i++){
      const p=passed[i];
      setGStatus({msg:`[${i+1}/${passed.length}] Pushing "${p.title.slice(0,40)}..."`,progress:Math.round(5+((i/passed.length)*90)),type:'working'});
      // Images already fetched at import time
      ...
      if(i<passed.length-1)await new Promise(r=>setTimeout(r,650));
    }
    setGStatus({msg:fail===0?`✅ All ${ok} products pushed to Shopify!`:`⚠️ ${ok} pushed, ${fail} failed. ${errs[0]||''}`,progress:100,type:'done'});
