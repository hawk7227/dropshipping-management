    ...
    <div className="products-dark min-h-screen p-4 lg:p-6" style={{background:'#080810',color:'#e4e4e7',fontFamily:"'DM Sans',-apple-system,sans-serif"}}>
      ...
      <div className="max-w-[1800px] mx-auto">
        ...
        {tab==='manual'&&(<div className="space-y-4">
          ...
          <div className="mt-2"><div className="flex items-center justify-between mb-4"><h3 className="text-sm font-bold text-white">📦 Shopify Products ({gridProducts.length})</h3><div className="flex items-center gap-2"><button onClick={async()=>{if(!confirm('Delete ALL products without images? This cannot be undone.'))return;setGridLoading(true);try{const r=await fetch('/api/products?action=delete-no-images',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({})});const d=await r.json();alert(d.message||`Deleted ${d.deleted} products`);await fetchGrid();}catch(e:any){alert('Error: '+e.message);}finally{setGridLoading(false);}}} className="px-3 py-1.5 rounded-lg text-[10px] font-semibold text-red-400 border border-red-500/30 hover:bg-red-500/10 transition-all">🗑️ Delete No-Image ({gridProducts.filter(p=>!p.image_url).length})</button><button onClick={async()=>{if(!confirm(`DELETE ALL ${gridProducts.length} PRODUCTS? This cannot be undone!`))return;if(!confirm('Are you SURE? This deletes everything.'))return;setGridLoading(true);try{const r=await fetch('/api/products?action=delete-all',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({})});const d=await r.json();alert(d.message||`Deleted ${d.deleted} products`);await fetchGrid();}catch(e:any){alert('Error: '+e.message);}finally{setGridLoading(false);}}} className="px-3 py-1.5 rounded-lg text-[10px] font-semibold text-red-400 border border-red-500/30 hover:bg-red-500/10 transition-all">⚠️ Delete All</button><ViewToggle viewMode={viewMode} onViewModeChange={setViewMode} gridDensity={gridDensity} onGridDensityChange={setGridDensity} productCount={gridProducts.length}/></div></div><ProductCardGrid products={gridProducts} density={gridDensity} isLoading={gridLoading} selectedIds={gridSelectedIds} onSelectToggle={gst} onSelectAll={gsa} onDeselectAll={gda} onViewDetails={gvd} onRefresh={grf} onPause={gpa} onRemove={grm} onSyncShopify={gss} onBulkSync={gbs} onBulkActivate={gba} onBulkPause={gbp} onBulkExport={gbe} onBulkArchive={gbr}/></div>
        </div>)}
