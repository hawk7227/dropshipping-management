...
⚙️ SELF-CORRECTIONS
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {[
                    ...
                    { job: 'competitor-prices', schedule: 'Every 60 min (6AM-11PM)', desc: 'Rainforest API price check + auto-adjust + auto-source', status: selectedNiche === 'evenbetterbuy' ? 'active' : 'disabled' },
  ...
  const NicheCommandView = ({ selectedNiche, niches, onNavigate }: { selectedNiche: string; niches: Niche[]; onNavigate: (tab: TabType) => void }) => {
    ...
    const configs: Record<string, any> = {
      ...
      evenbetterbuy: { kpis: [{l:'Orders',v:'0',t:'Growing',c:'#22c55e'},{l:'Revenue',v:'$0',t:'Growing',c:'#a855f7'},{l:'Feed',v:'256 issues',t:'0',c:'#ef4444'},{l:'Products',v:'316',t:'All',c:'#f59e0b'},{l:'Social',v:'0',t:'3/day',c:'#ec4899'},{l:'Price Rank',v:'TBD',t:'Lowest',c:'#8b5cf6'}], goal:'Fix feed, price leadership', day:'Ongoing', rules:['⚠️ 256 GMC feed issues','✅ Competitor price auto-check','✅ Undercut $0.01, floor price','✅ Feed rebuild after adjust','📊 316 titles need optimization','⚡ Auto-sourcing runs throughout the day'], learns:['$0.01 undercut = 40% more clicks','TT deal reveals = best engagement'], tests:['Price-first vs savings-first','Side-by-side vs standalone','Free ship $25/$35/$50'], social:[{t:'9:00 AM',p:'🎵 TT',k:'Deal of the day'},{t:'5:00 PM',p:'📸 IG',k:'Deal spotlight'},{t:'7:00 PM',p:'📘 FB',k:'Product showcase'}], seo:['Fix 256 feed errors','Optimize 316 titles','Activate free listings','Best [X] under $25 pages','Competitor monitoring'], sched:[{t:'6AM-11PM',s:'pricing',k:'Auto-source + price check (every 60min)'},{t:'6AM',s:'analytics',k:'Shopify + GMC data pull'},{t:'7AM',s:'seo',k:'Rebuild feed with new prices'},{t:'8AM',s:'gmc',k:'Diagnostics check'},{t:'9AM',s:'social',k:'TT deal of the day'},{t:'12PM',s:'pricing',k:'Midday price adjustment'},{t:'5PM',s:'social',k:'IG spotlight'},{t:'7PM',s:'social',k:'FB showcase'},{t:'9PM',s:'brain',k:'Goal check + daily report'}], vids:[{n:'Deal Reveal',ty:'Kinetic'},{n:'Deal Finder',ty:'Showcase'},{n:'Unboxing',ty:'Showcase'}], vault:[{l:'Social Hooks',c:8},{l:'SEO Keywords',c:20},{l:'Price Rules',c:3},{l:'Feed Fixes',c:256},{l:'Title Templates',c:316},{l:'A/B Tests',c:10},{l:'Video Templates',c:3}] },
    };
    ...
    return (
      <div className="space-y-3">
        {/* KPI Header */}
        <div className="bg-[#12121a] border border-[#a855f7]/30 rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(168,85,247,0.05), rgba(236,72,153,0.05))' }}>
          ...
          <div className="p-4 grid grid-cols-3 lg:grid-cols-6 gap-3">
            {d.kpis.map((k: any, i: number) => (
              <div key={i} className="bg-[#0d0d12] rounded-xl p-3 text-center border border-[#27272a]">
                <p className="text-2xl font-bold" style={{color:k.c}}>{k.v}</p><p className="text-[9px] text-white mt-0.5">{k.l}</p><p className="text-[8px] text-[#52525b]">→ {k.t}</p>
              </div>
            ))}
          </div>
          {/* Sourcing CTA for EvenBetterBuy */}
          {selectedNiche === 'evenbetterbuy' && (
            <div className="px-4 pb-4">
              <a href="/sourcing" className="block w-full py-3 text-center rounded-xl text-sm font-bold text-white transition-all hover:shadow-lg hover:shadow-cyan-500/20" style={{ background: 'linear-gradient(135deg, #06b6d4, #8b5cf6)' }}>
                🎯 Open Product Sourcing Engine — Source, Price, Push to Shopify
              </a>
            </div>
          )}
        </div>
