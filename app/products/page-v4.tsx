...
// app/products/page-v4.tsx
// ═══════════════════════════════════════════════════════════════════════════
// PRODUCTS PAGE V4 — PRODUCTION BUILD
// ═══════════════════════════════════════════════════════════════════════════
//
// Features:
...
//
// This file is a NEW file. It does not modify any existing files.
// To activate: rename page.tsx → page-legacy.tsx, rename page-v4.tsx → page.tsx
// ═══════════════════════════════════════════════════════════════════════════
...
function ProductsPageInner() {
  ...
  const handleBulkSync = useCallback(async (ids: string[]) => {
    try {
      ...
      if (result.success) {
        toast.success(`${ids.length} product${ids.length > 1 ? 's' : ''} synced to Shopify`);
        setSelectedIds(new Set());
        await fetchProducts(true);
      } else {
      ...
      toast.error('Bulk sync failed', msg);
      throw err;
    }
  }, [toast, fetchProducts]);
    ...
    <div className="space-y-5">
      ...
      {state.products.length > 0 && (
        <>
          ...
          {/* ── Sourcing Panel (collapsible) ─────────────────────────── */}
          {state.showSourcing && (
            <div className="animate-slide-up">
              <ManualSourcingBar onSourcingComplete={() => fetchProducts(false)} />
            </div>
          )}
