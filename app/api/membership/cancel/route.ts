/**
 * API Route: POST /api/membership/cancel
 * Cancels the user's membership at the end of the current billing period
 */

import { NextRequest, NextResponse } from 'next/server';
import { createRouteHandlerClient } from '@supabase/auth-helpers-nextjs';
import { cookies } from 'next/headers';
import { cancelSubscription } from '@/lib/stripe-products';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (membership.cancel_at_period_end) {
      return NextResponse.json(
        { error: 'Membership is already set to cancel' },
        { status: 400 }
      );
    }
    ...
    if (!['active', 'trialing'].includes(membership.status)) {
      return NextResponse.json(
        { error: 'Cannot cancel inactive membership' },
        { status: 400 }
      );
    }

    // Cancel subscription at period end (user keeps access until then)
    const updated = await cancelSubscription(membership.stripe_subscription_id);
    ...
    console.error('[api/membership/cancel] Error:', error);
    return NextResponse.json(
