...
async function searchRainforestAPI(params: DiscoverySearchParams): Promise<RainforestSearchResult[]> {
  ...
  console.log('[Discovery API] searchRainforestAPI called with:', {
    query: params.query,
  ...
  if (!apiKey) {
    ...
    console.log('[Discovery API] No API key, returning mock data');
    return generateMockResults(params);
  }

  try {
    // Build Rainforest API request
    // Note: Rainforest API expects prices in CENTS, not dollars
    ...
    // Add optional params
    if (params.category && CATEGORY_IDS[params.category]) {
      requestParams.append('category_id', CATEGORY_IDS[params.category]);
    }
    if (params.minPrice) {
      // Convert dollars to cents for Rainforest API
      requestParams.append('min_price', String(Math.round(params.minPrice * 100)));
    }
    if (params.maxPrice) {
      // Convert dollars to cents for Rainforest API
      requestParams.append('max_price', String(Math.round(params.maxPrice * 100)));
    }
    if (params.primeOnly) {
      requestParams.append('prime', 'true');
    }
    requestParams.append('page', String(params.page || 1));

    console.log('[Discovery API] Calling Rainforest:', `${RAINFOREST_API_URL}?type=search&search_term=${params.query}&...`);
    ...
    if (!response.ok) {
      console.error('[Discovery API] Rainforest response not ok:', response.status);
      throw new Error(`Rainforest API error: ${response.status}`);
    }

    const data = await response.json();
    console.log('[Discovery API] Rainforest returned', data.search_results?.length || 0, 'results');

    if (data.request_info?.success === false) {
      console.error('[Discovery API] Rainforest request failed:', data.request_info);
      throw new Error(data.request_info?.message || 'Rainforest request failed');
    }

    // Transform Rainforest response to our format
    const results = (data.search_results || []).map((item: Record<string, unknown>) => ({
      asin: item.asin as string,
      title: item.title as string,
      price: parseFloat(String((item.price as any)?.value || item.price || 0)),
      image_url: item.image as string,
      rating: parseFloat(String(item.rating || 0)),
      review_count: parseInt(String(item.ratings_total || 0)),
      category: ((item.categories as any)?.[0]?.name as string) || 'Unknown',
      is_prime: Boolean(item.is_prime),
      availability: ((item.availability as any)?.raw as string) || 'Unknown',
    }));

    console.log('[Discovery API] Transformed', results.length, 'products');
    return results;
  } catch (error) {
    console.error('[Discovery API] Rainforest API error:', error);
    // Fallback to mock data
    console.log('[Discovery API] Falling back to mock data');
    return generateMockResults(params);
  }
}
