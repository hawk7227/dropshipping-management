      ...
      switch (jobType) {
        ...
        case 'shopify-sync': {
          ...
          const { data: unsyncedProducts } = await supabase
            .from('products')
            .select('id, asin, title, brand, category, product_type, description, image_url, images, rating, review_count, retail_price, cost_price, status')
          ...
          if (unsyncedProducts && unsyncedProducts.length > 0) {
            ...
            for (const product of unsyncedProducts) {
              try {
                ...
                const comparePrice = product.cost_price && product.retail_price && product.retail_price > product.cost_price
                  ? (product.retail_price * 1.2).toFixed(2)
                  : undefined;
                ...
                const shopifyPayload = {
                  product: {
                    ...
                    metafields: [
                      ...
                      product.cost_price ? { namespace: 'custom', key: 'cost_price', value: product.cost_price.toString(), type: 'number_decimal' } : null,
                      product.rating ? { namespace: 'reviews', key: 'rating', value: product.rating.toString(), type: 'number_decimal' } : null,
                      product.review_count ? { namespace: 'reviews', key: 'count', value: product.review_count.toString(), type: 'number_integer' } : null,
                    ].filter(Boolean),
                  }
                };
