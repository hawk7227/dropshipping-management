  ...
  const testCampaign = async () => {
    ...
    try {
      const response = await fetch('/api/campaigns/test', {
        ...
        body: JSON.stringify({ 
          channels, 
          email: channels.email ? { subject: emailSubject, body: emailBody } : undefined,
          sms: channels.sms ? { message: smsMessage } : undefined,
          mms: channels.mms ? { message: mmsMessage } : undefined,
        })
      });
      ...
      if (result.success) {
        ...
        setSendStatus(`✗ Test failed: ${result.error || 'Unknown error'}`);
      }
    } catch (error) {
      setSendStatus('✗ Test failed: Network error');
    }
  };
  ...
  const sendCampaign = async () => {
    ...
    try {
      ...
      const recipients = leads.map(lead => ({
        name: lead.name,
        email: lead.email,
        phone: (lead as any).phone, // Add phone if available
      ...
      if (result.success) {
        ...
        setSendStatus(`✓ Campaign sent! ${summary.totalSuccess} successful, ${summary.totalFailed} failed`);
      } else {
        setSendStatus(`✗ Send failed: ${result.error || 'Unknown error'}`);
      }
    } catch (error) {
      setSendStatus('✗ Send failed: Network error');
    }
  };
