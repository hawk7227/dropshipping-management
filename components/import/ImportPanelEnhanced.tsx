...
function ReviewStep({
  ...
  const fetchPricePreview = async () => {
    ...
    try {
      // Preview first 10 ASINs to show prices
      const previewAsins = uniqueAsins.slice(0, 10);
      ...
      const response = await fetch('/api/import/v2', {
        ...
        body: JSON.stringify({
          ...
          mode: 'full_import',
          options: {
            dryRun: true, // Don't actually import
            preview: true,
          },
        }),
      });
      ...
      setPreviewData(data.data?.products || data.data?.preview || []);
    } catch (err) {
  ...
  const previewStats = previewData ? {
    avgPrice: previewData.reduce((sum, p) => sum + (p.amazon_price || p.price || 0), 0) / previewData.length,
    avgProfit: previewData.reduce((sum, p) => sum + (p.profit_amount || p.estimated_profit || 0), 0) / previewData.length,
    avgMargin: previewData.reduce((sum, p) => sum + (p.profit_percent || p.estimated_margin || 30), 0) / previewData.length,
    primeCount: previewData.filter(p => p.is_prime).length,
  } : null;

  return (
    <div className="space-y-6">
      <div className="text-center">
        <h3 className="text-lg font-semibold text-gray-900">Review & Preview Prices</h3>
        <p className="text-gray-500 mt-1">Preview prices before importing</p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        <div className="px-4 py-3 bg-gradient-to-r from-green-50 to-blue-50 border-b border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-gray-900">Price Preview</span>
          </div>
          {!previewData && (
            <button
              onClick={fetchPricePreview}
              disabled={isPreviewing || !mode}
        ...
        {!previewData && !previewError && (
          <div className="px-4 py-6 text-center text-gray-500">
            <p className="text-sm">Click "Preview Prices" to see estimated prices and profits before importing.</p>
            <p className="text-xs mt-1">(Uses ~10 Keepa tokens for preview)</p>
          </div>
        )}
          ...
          <>
            ...
            <div className="px-4 py-3 bg-green-50 border-b border-gray-200">
              <div className="grid grid-cols-4 gap-4 text-center">
                <div>
                  <p className="text-lg font-bold text-gray-900">${previewStats?.avgPrice.toFixed(2) || 'N/A'}</p>
                  <p className="text-xs text-gray-500">Avg Amazon Price</p>
                </div>
                <div>
                  <p className="text-lg font-bold text-green-600">${previewStats?.avgProfit.toFixed(2) || 'N/A'}</p>
                  <p className="text-xs text-gray-500">Avg Est. Profit</p>
                </div>
                <div>
                  <p className="text-lg font-bold text-blue-600">{previewStats?.avgMargin.toFixed(0) || '30'}%</p>
                  <p className="text-xs text-gray-500">Avg Margin</p>
                </div>
                <div>
                  <p className="text-lg font-bold text-purple-600">{previewStats?.primeCount || 0}/{previewData.length}</p>
                  <p className="text-xs text-gray-500">Prime Eligible</p>
                </div>
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 sticky top-0">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-right text-xs font-medium text-gray-500">Amazon</th>
                    <th className="px-4 py-2 text-right text-xs font-medium text-gray-500">Your Price</th>
                    <th className="px-4 py-2 text-right text-xs font-medium text-gray-500">Profit</th>
                    <th className="px-4 py-2 text-center text-xs font-medium text-gray-500">Rating</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {previewData.map((product, idx) => {
                    const amazonPrice = product.amazon_price || product.price || 0;
                    const yourPrice = amazonPrice * 1.7; // 70% markup
                    const profit = yourPrice - amazonPrice;
                    const margin = amazonPrice > 0 ? (profit / yourPrice) * 100 : 0;
                    ...
                    return (
                      <tr key={product.asin || idx} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 text-right">
                          ...
                          <span className="text-gray-400 text-xs ml-1">({margin.toFixed(0)}%)</span>
                        </td>
            ...
            {uniqueAsins.length > 10 && (
              <div className="px-4 py-2 bg-gray-50 text-center text-xs text-gray-500 border-t">
                Showing 10 of {uniqueAsins.length} products. All products will be imported with similar pricing.
              </div>
            )}
          </>
        )}
      ...
      <div className="flex justify-between">
        ...
        >
          ...
          Import {uniqueAsins.length} Products
        </button>
      </div>
    </div>
  );
