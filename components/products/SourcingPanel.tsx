...
export function SourcingPanel({ onSourcingComplete }: SourcingPanelProps) {
  ...
  const loadSettings = async () => {
    try {
      const { data } = await supabase.from('sourcing_settings').select('*').eq('id', '00000000-0000-0000-0000-000000000001').maybeSingle();
      if (data) {
        setF(p => ({ ...p,
  ...
  const loadSyncStats = async () => {
    try {
      const { data } = await supabase.from('products').select('shopify_product_id, shopify_synced_at, status');
      if (data) {
        const s = data.filter((p: any) => p.shopify_product_id).length;
        const f = data.filter((p: any) => p.status === 'sync_failed').length;
        const latestSync = data.filter((p: any) => p.shopify_synced_at).sort((a: any, b: any) => new Date(b.shopify_synced_at).getTime() - new Date(a.shopify_synced_at).getTime())[0];
        setSynced(s); setSyncFailed(f); setSyncPending(data.length - s);
        setLastSyncTime(latestSync?.shopify_synced_at ?? null);
      }
    } catch (e) { /* ignore */ }
  };
